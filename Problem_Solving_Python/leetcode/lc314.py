...
class Solution2:
    def verticalOrder(self, root: Optional[TreeNode]) -> List[List[int]]:
        if not root: return []
        node_map_by_level=defaultdict(list)
        q=deque()
        q.append([root,0])
        while q:
            node,level=q.popleft()
            node_map_by_level[level].append(node.val)
            if node.left: q.append([node.left,level-1])
            if node.right: q.append([node.right,level+1])
        return [node_map_by_level[level] for level in sorted(node_map_by_level)]
...
class MyTestCase(unittest.TestCase):
    ...
    def test07(self):
        self.assertEqual([], get_sol().verticalOrder(des([])))