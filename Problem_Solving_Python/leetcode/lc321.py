...
class Solution:
    ...
    def largest_arr(self,nums,k):
        ...
        for i in range(len(nums)):
            ...
            while n-i>k-len(st) and st and st[-1]<nums[i]:
                st.pop()
            st.append(nums[i])
        while len(st)>k: st.pop() # remove extra elements. not required if cnt is used when merging
        return st
    ...
    def maxNumber(self, nums1: List[int], nums2: List[int], k: int) -> List[int]:
        def merge(nums1,nums2,length): # take 'length' no of elements from nums1 and 'k - length' no of elemnts from nums2
            ...
            i=j=0
            # cnt=0 # -> if we add this line "while len(st)>k: st.pop()", then cnt is not required
        ...
        for i in range(1,k+1):
            # res=max(res,merge(nums1,nums2,i),key=listCmp) # alternative

            if len(nums1)<i or len(nums2)<k-i: continue
            res=max(res,merge(nums1,nums2,i))
        return res
...
class MyTestCase(unittest.TestCase):
    ...
    def test00a6(self):
        self.assertEqual([9,5,8,3], get_sol().largest_arr([9,1,2,5,8,3],4))
    def test01(self):
