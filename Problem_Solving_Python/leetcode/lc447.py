...
class Solution3:
    def numberOfBoomerangs(self, points: List[List[int]]) -> int:
        ...
        for i in range(n):
            ...
            for j in range(n):
                if j==i: continue
                d = sq_dist(points[i],points[j])
                di[d]+=1
            for d in di:
                if di[d]>1:
                    # number of ways to select 2 from n = nP2 = n!/(n - 2)! = n * (n - 1)
                    cnt+=di[d]*(di[d]-1)
        return cnt
class Solution2:
    # tle
        ...
        for i in range(n):
            for j in range(n):
                ...
                for k in range(n):
                    ...
                    if sq_dist(points[i],points[j])==sq_dist(points[i],points[k]):
                        cnt+=1
        return cnt
...
class tester(unittest.TestCase):
    def test1(self):
        points = [[0,0],[1,0],[2,0]]
        Output= 2
        self.assertEqual(Output,get_sol().numberOfBoomerangs(points))
    def test2(self):
        points = [[1,1],[2,2],[3,3]]
        Output= 2
        self.assertEqual(Output,get_sol().numberOfBoomerangs(points))
