...
class Solution:
    ...
    # Write a sub function s(TreeNode root) to get the sum of a sub tree. s is short for sub and sum.
    ...
    def maxProduct(self, root: TreeNode) -> int:
        ...
        def traverse(root:TreeNode):
            # update self.maxx after
                            # 1. adding left
                            # 2. adding root and right
            # or update self.maxx after
                            # 1. adding right
                            # 2. adding left and root
            if not root: return 0
            cur_sum=0

            cur_sum+=traverse(root.left)
            self.maxx=max(self.maxx,(total_sum-cur_sum)*cur_sum) # 1. updating after adding left

            cur_sum+=root.val
            cur_sum+=traverse(root.right)
            self.maxx=max(self.maxx,(total_sum-cur_sum)*cur_sum) # 2. updating after adding right and root
        ...
        total_sum = sum_tree(root) # sum of the entire tree
        traverse(root)
        return self.maxx % MOD
...
class tester(unittest.TestCase):
    def test_1(self):
        ...
        Output= 110
        self.assertEqual(Output, get_sol().maxProduct(root))
    def test_2(self):
