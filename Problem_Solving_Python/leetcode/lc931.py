...
class Solution:
    def minFallingPathSum(self, matrix: List[List[int]]) -> int:
        ...
        def dfs(i,j):
            if i==m: return 0
            if j>=n or j<0: return float('inf')
            if (i,j) in dp: return dp[(i,j)]
            minn = matrix[i][j]+min(dfs(i+1,j-1),dfs(i+1,j),dfs(i+1,j+1))
            dp[(i,j)] = minn
            return minn
        minn=float('inf')
        for j in range(n):
            minn=min(minn,dfs(0,j))
        return minn
class Solution2:
    # iterative
    def minFallingPathSum(self, matrix: List[List[int]]) -> int:
...
class tester(unittest.TestCase):
    def test1(self):
        ...
        Output= 13
        self.assertEqual(Output,get_sol().minFallingPathSum(matrix))
    def test2(self):
        matrix = [[-19,57],[-40,-5]]
        Output= -59
        self.assertEqual(Output,get_sol().minFallingPathSum(matrix))
