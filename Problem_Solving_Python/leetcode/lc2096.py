...
class Solution:
    def getDirections(self, root: TreeNode, startValue: int, destValue: int) -> str:
        def dfs(node,dest,path):
            ...
            if node.val==dest:
                path.appendleft('#')
            elif dfs(node.left,dest,path):
                path.appendleft('L')
            elif dfs(node.right,dest,path):
                path.appendleft('R')
            return path

        path1 = dfs(root,startValue,deque())
        path2 = dfs(root,destValue,deque())
        # print(path1)
        # print(path2)
        path1.pop()
        path2.pop()
...
class tester(unittest.TestCase):
    ...
    def test6(self):
        Output= "U"
        self.assertEqual(Output,get_sol().getDirections(deserialize("5,8,3,1,null,4,7,6,null,null,null,null,null,null,2"), 4, 3))
